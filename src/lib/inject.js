  ...
  return fse
    ...
    .then((zip) =>
      BbPromise.resolve(
        ...
        .map(([file, relativeFile]) => Promise.all([file, relativeFile, fse.statAsync(file)]))
...
function injectAllRequirements(funcArtifact) {
  ...
  this.context.debug('Injecting required Python packages to package...')

  if (this.serverless.service.package.individually) {
    return BbPromise.resolve(this.targetFuncs)
      .filter((func) =>
        (func.runtime || this.serverless.service.provider.runtime).match(/^python.*/)
      )
      .map((func) => {
        if (!get(func, 'module')) {
          set(func, ['module'], '.')
      ...
      .map((func) => {
        if (func.module !== '.') {
          ...
          return moveModuleUp(artifact, newArtifact, func.module).then(() => func)
        }
        return func
      })
      .map((func) => {
