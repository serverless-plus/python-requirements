...
function findTestFile(servicePath) {
  ...
  throw new Error('Unable to find serverless.{yml|yaml|json} or requirements.txt for getBindPath()')
}
...
function tryBindPath(serverless, bindPath, testFile) {
  const options = ['run', '--rm', '-v', `${bindPath}:/test`, 'alpine', 'ls', `/test/${testFile}`]
  try {
 ...
 */
function getBindPath(context, servicePath) {
  // Determine bind path
  ...
  if (baseBindPath[1] == ':') {
    ...
    pathStr = baseBindPath.substring(3)
  ...
  bindPaths.push(`/${drive.toLowerCase()}/${pathStr}`) // Docker Toolbox (seems like Docker for Windows can support this too)
  bindPaths.push(`${drive.toLowerCase()}:/${pathStr}`) // Docker for Windows
  // other options just in case
  bindPaths.push(`/${drive.toUpperCase()}/${pathStr}`)
  bindPaths.push(`/mnt/${drive.toLowerCase()}/${pathStr}`)
  bindPaths.push(`/mnt/${drive.toUpperCase()}/${pathStr}`)
  bindPaths.push(`${drive.toUpperCase()}:/${pathStr}`)
  ...
  for (let i = 0; i < bindPaths.length; i++) {
    ...
    if (tryBindPath(context, bindPath, testFile)) {
      return bindPath
    }
  }
