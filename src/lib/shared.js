 ...
 * @return {undefined}
 */
function checkForAndDeleteMaxCacheVersions(options, context) {
  ...
  if (
    ...
    const files = glob.sync([path.join(getUserCachePath(options), '*_slspyc/')], { mark: true })
    ...
    if (files.length >= options.staticCacheMaxVersions) {
      ...
      let items = 0
      for (let i = 0; i < files.length - options.staticCacheMaxVersions + 1; i++) {
        rimraf.sync(files[i])
        items++
      }
      // Log the number of cache files flushed
      context.debug(`Removed ${items} items from cache because of staticCacheMaxVersions`)
    }
  }
}
 ...
 * @return {string}
 */
function getRequirementsWorkingPath(subfolder, requirementsTxtDirectory, options) {
